...
class Token
{
    ...
    public static function connect(array $options = [], $name = false)
    {
        ...
        if (true === $name || !isset(self::$instance[$name])) {
            ...
            // 重组options
            if($type === 'mysql'){
                $temp = ['table' => self::$options['auth_token']];
                $options = array_merge($options, $temp);
            }

            if ($type === 'redis'){
                $temp = ['userprefix' => self::$options['auth_token'] . '_up:' ,'tokenprefix' => self::$options['auth_token'] . '_tp:'];
                $options = array_merge($options, Config::get('redis.'), $temp);
            }
